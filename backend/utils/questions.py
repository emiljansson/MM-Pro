...
def generate_graphs(min_val: int, max_val: int, lang: str = "sv") -> Dict[str, Any]:
    """
    Graphs - read values from line graphs:
    
    Easy: Simple linear functions, find y for given x
    Medium: Find x for given y, steeper lines
    Hard: Non-linear patterns, interpolation
    """
    texts = {
        "sv": {
            "find_y": "Vad är y när x = {x}?",
            "find_x": "Vad är x när y = {y}?",
            "find_y_at": "Avläs y-värdet vid x = {x}",
            "find_x_at": "Avläs x-värdet vid y = {y}",
            "slope": "Hur mycket ökar y när x ökar med 1?",
            "intercept": "Var skär linjen y-axeln?",
        },
        "en": {
            "find_y": "What is y when x = {x}?",
            "find_x": "What is x when y = {y}?",
            "find_y_at": "Read the y-value at x = {x}",
            "find_x_at": "Read the x-value at y = {y}",
            "slope": "How much does y increase when x increases by 1?",
            "intercept": "Where does the line cross the y-axis?",
        }
    }
    t = texts.get(lang, texts["sv"])
    ...
    # Generate line parameters: y = slope * x + intercept
    if is_easy:
        slope = random.choice([1, 2])
        intercept = random.randint(0, 3)
        x_range = list(range(0, 6))  # x from 0 to 5
    elif is_medium:
        slope = random.choice([1, 2, 3, -1, -2])
        intercept = random.randint(-2, 5)
        x_range = list(range(-2, 7))  # x from -2 to 6
    else:  # hard
        slope = random.choice([1, 2, 3, -1, -2, -3])
        intercept = random.randint(-5, 5)
        x_range = list(range(-3, 8))  # x from -3 to 7
    ...
    if is_easy:
        q_type = random.choice(['find_y', 'find_y_at'])
    elif is_medium:
        q_type = random.choice(['find_y', 'find_x', 'find_y_at', 'slope'])
    else:
        q_type = random.choice(['find_y', 'find_x', 'slope', 'intercept'])
    
    if q_type == 'find_y' or q_type == 'find_y_at':
        # Pick a valid x value
        valid_x = [p["x"] for p in points if -5 <= p["y"] <= 15]
        if not valid_x:
            valid_x = [p["x"] for p in points]
        x_val = random.choice(valid_x)
        y_val = slope * x_val + intercept
        
        question_text = t[q_type].format(x=x_val)
        answer = y_val
        
    elif q_type == 'find_x' or q_type == 'find_x_at':
        # Pick a y value that has a clean x
        valid_points = [p for p in points if -5 <= p["y"] <= 15]
        if not valid_points:
            valid_points = points
        ...
        question_text = t.get(q_type, t["find_x"]).format(y=y_val)
        answer = x_val
        
    elif q_type == 'slope':
        question_text = t["slope"]
        answer = slope
        
    else:  # intercept
        question_text = t["intercept"]
        answer = intercept
